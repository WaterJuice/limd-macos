...
package first by running: ${YELLOW}${BREW_OR_PORTS_UNINSTALL} $PKG${NORMAL}"
    ...
    case "$response" in
      [rR])
        echo -e "Running ${YELLOW}$BREW_OR_PORTS_UNINSTALL $PKG${NORMAL}..."
        $BREW_OR_PORTS_UNINSTALL $PKG 2>/dev/null || exit 1
        ;;
      [cC])
        ;;
      ""|*)
...
for I in $COMPONENTS; do
  ...
  if test -d "$COMP/.git" && ! test -f "$COMP/.git/shallow"; then
    ...
    if test -z "`git branch |grep '$CVER'`"; then
      git checkout $CVER --quiet || (echo "Failed to check out $CVER" ; exit 1)
    fi
    ...
    git pull --quiet || (echo "Failed to pull from git" ; exit 1)
    cd "$CURDIR"
  else
    rm -rf $COMP
    if test "$CVER" != "master"; then
      echo "Cloning $COMP (release $CVER)";
      git clone -b $CVER https://github.com/libimobiledevice/$COMP 2>/dev/null || (echo "Failed to clone $COMP" ; exit 1)
    else
      echo "Cloning $COMP (master)";
      git clone https://github.com/libimobiledevice/$COMP 2>/dev/null || (echo "Failed to clone $COMP" ; exit 1)
    fi
  fi
done
fi
...
LIBCURL_VERSION=`curl-config --version |cut -d " " -f 2`
LIBXML2_VERSION=`xml2-config --version |cut -d " " -f 2`
...
./autogen.sh --prefix="$PREFIX" \
  libcurl_CFLAGS="-I$SDKDIR/usr/include" libcurl_LIBS="-lcurl" libcurl_VERSION="$LIBCURL_VERSION" \
  libxml2_CFLAGS="-I$SDKDIR/usr/include" libxml2_LIBS="-lxml2" libxml2_VERSION="$LIBXML2_VERSION" \
  libimobiledevice_CFLAGS="$LIMD_CFLAGS" libimobiledevice_LIBS="$LIMD_LIBS" libimobiledevice_VERSION="$LIMD_VERSION" \
  libplist_CFLAGS="$LIBPLIST_CFLAGS" libplist_LIBS="$LIBPLIST_LIBS" libplist_VERSION="$LIBPLIST_VERSION" \
  > "$LOGF" 2>&1 || error_out
echo -e "[*] Building..."
